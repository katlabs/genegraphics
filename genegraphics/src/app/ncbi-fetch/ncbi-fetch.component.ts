...
import { getDataFetchesObservable } from '../utils';
import { DatabaseService } from '../database.service';
import { GeneGraphic, NCBIGenome } from '../models';
...
export class NcbiFetchComponent implements OnInit {
  ...
  private _filter(value: string): NCBIGenome[] {
    if(value && this.options && typeof value==='string'){
      ...
      return this.options.filter(option => option.organism && option.organism.toLowerCase().includes(filterValue)).slice(0,10);
  ...
  fetchFromNcbi(){
    ...
    if(term){
      this.loading = true;
      this.ncbiFetch.getGBFile(type_str, term, regArr)
  ...
  ngOnInit(): void {
    ...
    this.filteredGenomes = this.ncbiForm.get('organismCtrl')?.valueChanges.pipe(startWith(''),
    map(value=>this._filter(value||'')),
    );
    this.searchTypeCtrl.valueChanges
      .subscribe(()=>{
      if(this.error_msg){
        this.error_msg = null;
      }
