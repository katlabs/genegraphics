...
import { NCBIGenome, DataFetch } from './models'
import { DatabaseService } from './database.service'
import { HttpClient, HttpHeaders } from '@angular/common/http'
import { from, interval, Observable, of, throwError } from 'rxjs'
import { catchError, concatMap, filter, first, map, retry, switchMap, take, timeout } from 'rxjs/operators'
...
export class NcbiFetchService {
  ...
  parseUniprotJobID(responseObj:any): Observable<string>{
    if(!responseObj.jobId) throw new Error("Could not post job to Uniprot API");
  ...
  pollUniprotJobResults(jobId:string): Observable<string>{
    const pollURL = 'https://rest.uniprot.org/idmapping/status/'
    + jobId;
  ...
  getGBFile(searchType: string, term: string, seqArr: [number, number]) {
    if (searchType === 'Gene ID') {
      ...
      concatMap(response=>this.parseGeneIDFetchResponse(response, seqArr[1])),
      concatMap(parsed=>this.ncbiFetch(parsed.fetchID,'nuccore','text','gbwithparts',[
          parsed.start,
          parsed.stop
        ])))
    } else {
      ...
      complete: () => {
        ...
        if(clearOld){
          this.db.dataFetches.toArray().then(arr=>{
            if (arr.length>1){
              let old = arr.reduce((oldest, data)=> oldest.last_fetch < data.last_fetch ?
              oldest : data)
