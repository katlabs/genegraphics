...
export async function changeRegionPosition(db: DatabaseService, region: Region, geneGraphic:GeneGraphic, dir:string){
  const curr_pos = region.position
    let curr_pos_ind = geneGraphic.regions.map(r=>r.position).indexOf(curr_pos)
  if(dir==="down"){
    let next_pos_ind = geneGraphic.regions.map(r=>r.position).indexOf(curr_pos+1)
    geneGraphic.regions[curr_pos_ind].position = curr_pos+1;
    geneGraphic.regions[next_pos_ind].position = curr_pos;
  } else if(dir==="up"){
    let prev_pos_ind = geneGraphic.regions.map(r=>r.position).indexOf(curr_pos-1)
    geneGraphic.regions[curr_pos_ind].position = curr_pos-1;
...
export function getCurrentSavedSelections(db: DatabaseService) {
  return liveQuery(()=>db.transaction('rw',db.geneGraphics,db.selections,async()=>{
    const open_geneGraphic = await db.geneGraphics.orderBy('opened').last();
    if(open_geneGraphic?.id){
      return db.selections.where({geneGraphicId: open_geneGraphic.id}).toArray();
    } else{
...
export async function saveNewSelection(db: DatabaseService, geneGraphicId:number, selection:Selection){
  const newSelection = {
    geneGraphicId: geneGraphicId,
    selection: selection
  }
  return db.selections.add(newSelection).then(d=>console.log(d)).catch(err=>console.log(err));
}

export async function deleteSelection(db: DatabaseService, geneGraphicId:number, selection:Selection){
  return await db.transaction('rw', db.selections, async()=>{
    const selections = await db.selections.where('geneGraphicId').equals(geneGraphicId).toArray();
    const del_id = selections.find(sel=>selectionsEqual(sel.selection,selection))?.id;
    if(del_id) return db.selections.delete(del_id);
    else throw new Error("Cannot delete: Selection not found");
  })
}
...
export function deleteFeatures(db:DatabaseService, geneGraphic: GeneGraphic, ids:string[]){
  console.log("delete feature ids: " + ids.join(","))
...
export function deleteRegions(db:DatabaseService, geneGraphic: GeneGraphic, ids:string[]){
  let newRegions: Region[] = []
  geneGraphic.regions.forEach((region)=>{
    if(!ids.includes(region.id)) newRegions.push(region);
  })
  geneGraphic.regions = newRegions;
  geneGraphic.regions.sort((r1,r2)=>r1.position-r2.position).forEach((r,i)=>r.position=i+1);
  updateScaleRatio(geneGraphic);
  db.geneGraphics.put(geneGraphic);
}

export function updateRegionLines(db:DatabaseService, geneGraphic:GeneGraphic, ids:string[], value:number){
  geneGraphic.regions.forEach(region=>{
...
export function flipRegionsAndUpdate(db: DatabaseService, geneGraphic: GeneGraphic, region_ids: string[], flip:boolean){
...
export function selectionsEqual(sel1:Selection, sel2:Selection) {
  ...
  return sel1_set.length != sel2_set.length ? false : sel1_set.every(x => sel2_set.includes(x));
}

export function getDefaultProperty(items:GeneGraphic[]|Region[]|Feature[], property:string){
  if(property in items[0]){
    const key = property as keyof typeof items[0];
    return items.map(i=>i).every((a, _, [b])=>a[key]===b[key]) ? items[0][key] : null;
  } else if(property in items[0].nameProps) {
    const key = property as keyof TextProps;
    return items.map(i=>i.nameProps).every((a, _, [b])=>a[key]===b[key]) ? items[0].nameProps[key] : DEFAULT_TEXTPROPS[key];
  } else{
    return null;
  }
}

export function updateTextProp(item: GeneGraphic|Region|Feature, field: string, newVal: any){
  switch (field){
    case 'hide':
...
export function updateTextProps(db: DatabaseService, type: string, geneGraphic: GeneGraphic, ids: any[], newVals: Record<string,any>){
  if (type==='geneGraphic'){
...
export function updateName(db: DatabaseService, type: string, geneGraphic: GeneGraphic, ids: any[], newName:string){
...
export function updateNameFromField(db:DatabaseService, type:string, geneGraphic:GeneGraphic, ids: any[], field: string){
  geneGraphic.regions.forEach(region=>{
    if(type==='region' && ids.includes(region.id)){
...
export function updateFeatureShapes(db:DatabaseService, geneGraphic:GeneGraphic, ids: string[], newShape:string){
  geneGraphic.regions.forEach(region=>{
    region.features.forEach(feature=>{
...
export function updateFeatureColors(db:DatabaseService, geneGraphic:GeneGraphic, ids: string[], color1:string, color2:string|null){
  geneGraphic.regions.forEach(region=>{
    region.features.forEach(feature=>{
      if(ids.includes(feature.id)){
...
export function getLanesSize(region: Region, featureHeight: number){
  ...
  const lane2 = region.lanes===2? (featureHeight*1.2)+lane2_bottom+lane2_top : 0
  return {total:lane1+lane2, lane1:lane1, lane2:lane2, lane1_top:lane1_top, lane2_top: lane2_top };
}
